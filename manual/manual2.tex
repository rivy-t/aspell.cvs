...
spell checker. Its main feature is that it does a much better job
of coming up with possible suggestions than just about any other spell
checker, including Ispell and Microsoft Word.
...
The latest version can always be found at GNI Aspell's home page at
\url{http://aspell.net}.
...
\char`\"{}aspell\char`\"{} program must be in your path in order for
the dictionaries to install correctly.
...
language names have changed to the two letter iso code. These means
that the your old personal dictionaries will not be recognized. However,
you can import the old dictionaries by running the {}``aspell-import''
script.
...
The format of the complied dictionaries has almost certainly changed.
So you will need to recompile them for the new Aspell.

{[}FIXME Add now about missing .pwli files{]}
...
{[}FIXME: Add note about nameing convenstions somewhere{]}{[}
...
\begin{itemize}
\item Changed the way language are handles. Languages are now two-letter
abbreviation
\item Added dvorak keymap.
\item Added ability to list the available dictionaries
\item Improved the check interface a bit.
